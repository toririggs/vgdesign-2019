...
//		return true;//within(other->x,other->y) || within(other->x+them.w,other->y) || within(other->x+them.w,other->y+them.h) || within(other->x, other->y+them.h);
//	}
...
class MyGame:public Game {
	...
	void init() {
		...
		//ball=new Sprite(this,"Frame1.bmp",320.0,240.0,20.0,-20.0,0.0,20.0);
		for (int i=0;i<2;i++) {
...
//				sprites[0]->setVelocity(sprites[0]->getVelocityX(),sprites[0]->getVelocityY()-200);  //now these can be changed to setPosition or setAcceleration
//			if (event.key.keysym.sym==SDLK_a) 
//				sprites[0]->setVelocity(sprites[0]->getVelocityX()+200,sprites[0]->getVelocityY());  
		//  }
	   // }
	}
	void update(float dt) {
//		for (unsigned int i=0;i<sphere->getsize();i++)
//			for (unsigned int j=i+1;j<sphere.size();i++)
//				if (!dead && sphere->collidesWith(sprites[j]))
//					sprites[j]->kill();
		...
		for (int x=200;x<1146;x++) {
			...
			for (int i = 400; i < 1500; i +=74){
				for (int j = 295; j < 800; j+=74) {
					...
					//if (count < 25) {b1->render(this,x+295,300);} //start go right
					//else if (count < 50) {b6->render(this,x+295,300);}
					//else if (count < 75) {b5->render(this,x+295,300);}
					//else if (count < 100) {b4->render(this,x+295,300);}
					//else if (count < 125) {b3->render(this,x+295,300);}
					//else if (count < 150) {b2->render(this,x+295,300);}
					//else {count = 0;} // end go right
				}
			}
	...
	void spheregoright() 
	{
		...
		if (passX != 1441) {
			...
			for (passX;passX<1442;passX++) 
			{
				...
				goal->render(this,goalX,goalY);
				sphere->render(this,passX,passY);
				if (count < 25) {b1->render(this,passX,passY);} //start go right
	...
	void spheregoleft()
	{
		...
		if (passX != 405) {
			...
			for (passX;passX>405;passX--) 
			{
				...
				goal->render(this,goalX,goalY);
				sphere->render(this,passX,passY);
				if (count < 25) {b1->render(this,passX,passY);} //start go left
				else if (count < 50) {b6->render(this,passX,passY);}
				else if (count < 75) {b5->render(this,passX,passY);}
				else if (count < 100) {b4->render(this,passX,passY);}
				else if (count < 125) {b3->render(this,passX,passY);}
				else if (count < 150) {b2->render(this,passX,passY);}
				else {count = 0;} // end go left
				//for (unsigned int i=0;i<sprites.size();i++)
				//{	sprites[i]->render(this);}
				count++;
				SDL_RenderPresent(renderer);
	...
	void spheregoup()
	{
		...
		if (passY != 300) {
			...
			for (passY;passY>300;passY--) 
			{
				...
				goal->render(this,goalX,goalY);
				sphere->render(this,passX,passY);
				if (count < 25) {b1->render(this,passX,passY);} //start go up
				else if (count < 50) {b6->render(this,passX,passY);}
				else if (count < 75) {b5->render(this,passX,passY);}
				else if (count < 100) {b4->render(this,passX,passY);}
				else if (count < 125) {b3->render(this,passX,passY);}
				else if (count < 150) {b2->render(this,passX,passY);}
				else {count = 0;} // end go up
				//for (unsigned int i=0;i<sprites.size();i++)
				//{	sprites[i]->render(this);}
				count++;
				SDL_RenderPresent(renderer);
	...
	void spheregodown()
	{
		...
		if (passY != 739) {
			...
			for (passY;passY<744;passY++) 
			{
				...
				goal->render(this,goalX,goalY);
				sphere->render(this,passX,passY);
				if (count < 25) {b1->render(this,passX,passY);} //start go down
				else if (count < 50) {b6->render(this,passX,passY);}
				else if (count < 75) {b5->render(this,passX,passY);}
				else if (count < 100) {b4->render(this,passX,passY);}
				else if (count < 125) {b3->render(this,passX,passY);}
				else if (count < 150) {b2->render(this,passX,passY);}
				else {count = 0;} // end go down
				//for (unsigned int i=0;i<sprites.size();i++)
				//{	sprites[i]->render(this);}
				count++;
				SDL_RenderPresent(renderer);
